...
<img src="https://chua-n.gitee.io/blog-images/notebooks/Java/33.png" alt="33" style="zoom:67%;" />
...
如果局部变量被匿名内部类访问，该局部变量相当于自动使用了final修饰（事实上，在Java8之前，Java要求被局部内部类、匿名内部类访问的局部变量必须显式使用final修饰，而Java8将这个操作隐式化了，并称之为"effectively final"，意思是对于被匿名内部类访问的局部变量，可以用final修饰也可以不用，但必须按照有final修饰的方式来用）。
