...
![img](https://chua-n.gitee.io/blog-images/notebooks/Java/54.png)

## 2. Executors工厂类

### 2.1 Executors类的静态方法

Java提供了一个<font size=5>`Executors`工厂类</font>来产生线程池，其包含如下几个静态方法来创建线程池：
...
- `ExecutorService`接口代表一个线程池，它可以执行Runnable/Callable对象所代表的线程，`ExecutorServic`同时代表尽快执行线程的线程池，只要线程池中有空闲线程，就立即执行线程任务；
...
`ExecutorService`接口是`Executor`接口的子接口，其提供了3个方法：
...
`ScheduledExecutorService`接口又是`ExecutorService`的子接口，其提供了4个方法：
