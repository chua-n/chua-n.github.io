...
简而言之，**反射**就是把Java类中的各种成分映射成一个个的Java对象。例如：一个类有包、构造方法、成员变量、方法等等信息，利用反射技术就可以对这个类进行解剖，把这些个组成部分映射成一个个对象。
...
<img src="https://chua-n.gitee.io/blog-images/notebooks/Java/image-20220111135844862.png" alt="image-20220111135844862" style="zoom:50%;" />
...
- 方法
    ...
    | 方法                                                         | 作用                                           |
    | ------------------------------------------------------------ | ---------------------------------------------- |
    | `Method getMethod(String name, Class<?>… parameterTypes)`    | 返回带指定形参列表的public方法                 |
    | `Method[] getMethods()`                                      | 返回类的所有public方法                         |
    | `Method getDeclaredMethod(String name, Class<?>… parameterTypes)` | 返回带指定形参列表的方法，与方法的访问权限无关 |
    | `Method[] getDeclaredMethods()`                              | 返回类的全部方法，与方法的访问权限无关         |
...
Class对象可以获得类里的构造器（`Constructor`）、方法（`Method`）、成员变量（`Field`），程序可以通过`Constructor`对象来调用对应的构造器创建实例，通过`Method`对象来执行对应的方法，通过`Field`对象直接访问并修改对象的成员变量值。这样一来，Java程序可以通过反射获取的这些对象得到类的相应成分的信息，同时可通过反射来生成并操作对象。

> 通过反射创建对象时性能要稍低一些，实际上，只有当程序需要动态创建某个类的对象时才会考虑反射，通常在开发通用性比较广的框架、基础平台时可能会大量使用反射。

`Constructor, Method, Field`的类图：

<img src="https://chua-n.gitee.io/blog-images/notebooks/Java/image-20220111113716392.png" alt="image-20220111113716392" style="zoom:50%;" />

<img src="https://chua-n.gitee.io/blog-images/notebooks/Java/image-20220111113800144.png" alt="image-20220111113800144" style="zoom:50%;" />

<img src="https://chua-n.gitee.io/blog-images/notebooks/Java/image-20220111113815389.png" alt="image-20220111113815389" style="zoom:50%;" />

由上可见，`Constructor, Method, Field`都实现了`java.lang.reflect.Member`接口，同时有一个共同的父类`AccessibleObject`。在`AccessibleObject`基类中，有一个`setAccessible(boolean flag)`方法，可用于将原本被private修饰的私有成员的权限放开，使得子类`Constructor, Method, Field`等可以获得所有相应类成员的访问权限。
...
- `Object invoke(Object obj, Object… args)`：其中`obj`是执行该方法的主调，后面的`args`是执行该方法时传入该方法的实参
...
| 方法                          | 作用                                                         |
| ----------------------------- | ------------------------------------------------------------ |
| `getXxx(Object obj)`          | 获取 obj 对象的该成员变量（基本类型）的值，此处Xxx对应8种基本类型 |
| `get(Object obj)`             | 获取 obj 对象的该成员变量（引用类型）的值                    |
| `setXxx(Object obj, Xxx val)` | 将 obj 对象的该成员变量（基本类型）设置成 val 值，此处Xxx对应8种基本类型 |
| `set(Object obj, Xxx val)`    | 将 obj 对象的该成员变量（引用类型）设置成 val 值             |
...
| `Type getGenericType()` | 获取成员变量的泛型类型 |
...
为了获得指定成员变量的泛型类型，应先使用`Type gType = field.getGenericType();`来获取该成员变量的泛型类型，然后将`Type`对象强转为`ParameterizedType`对象，`ParameterizedType`代表参数化的类型，也即增加了泛型限制的类型。`ParameterizedType`类提供了如下两个方法：
