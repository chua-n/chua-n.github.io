...
- **表锁(table lock)** ：锁定整张表，是MySQL中最基本也是开销最小的锁策略。
...
- **响应时间**：响应时间是两部分之和（然而遗憾的是，我们无法把响应时间真正细分到所谓的这些部分）：
  ...
  - 排除时间：指服务器因为等待某些资源而没有真正执行查询的时间。比如等待I/O操作完成，等待行锁......
...
<img src="https://figure-bed.chua-n.com/数据库/MySQL/19.jpg" alt="MySQL查询的执行路径" style="zoom:50%;" />
