...
因而观察者模式又称发布-订阅模式（Publish-Subscribe：Pub/Sub），它是一种通知机制，让发送通知的一方（被观察方）和接收通知的一方（观察者）能彼此分离，互不影响。

## 2. 示例（廖雪峰）

假设一个电商网站，有多种`Product`（商品），同时，`Customer`（消费者）和`Admin`（管理员）对商品上架、价格改变都感兴趣，希望能第一时间获得通知。于是，第一反应 `Store`（商场）可以这么写：
...
因此，上述问题的本质是`Store`希望发送通知给那些关心`Product`的对象，但`Store`并不想知道这些人是谁。<u>观察者模式就是要分离被观察者和观察者之间的耦合关系。</u>

其实要实现这一目标也很简单，`Store`不能直接引用`Customer`和`Admin`，相反，它引用一个`ProductObserver`接口，任何人想要观察`Store`，只要实现该接口，并且把自己注册到`Store`即可：
...
观察者模式也有很多变体形式。有的观察者模式把被观察者也抽象成接口：
...
在Tomcat中观察者模式有多处使用，如控制组件生命周期的`Lifecycle`就是这种模式的体现，还有对`Servlet`实例的创建、`Session`的管理、`Container`等都是同样的原理：

<img src="https://chua-n.gitee.io/figure-bed/notebook/Java/67.png" style="zoom:67%;" />
