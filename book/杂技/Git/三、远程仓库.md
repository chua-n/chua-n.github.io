...
## 10. 浅克隆（Shallow Clone）

在克隆远程仓库时，有些时候远程仓库的内容可能太多了，默认情况下 git 无法完全下载下来，会报如下错误：

```cmd
D:\>git clone https://github.com/user/repo.git
Cloning into 'repo'...
remote: Enumerating objects: 136113, done.
...
这个时候可以使用 `git clone` 命令的 `--depth` 参数，其表示只克隆指定深度的历史记录，而不是完整的仓库历史。比如设定 `--depth=1`，当克隆完成后，可以再通过 `git fetch --unshallow` 命令来获取完成历史。示意如下：

```bash
git clone --depth 1 https://github.com/user/repo.git # 只克隆最新一次提交的代码，没有历史提交记录
cd repo
git fetch --depth=10  # 获取最近10次提交
git fetch --depth=100 # 获取最近100次提交
git fetch --unshallow # 获取完整历史（谨慎使用，会下载全部数据）
```

然而，使用浅克隆的 `--depth` 参数会有一个副作用，为节省空间，克隆下来的本地仓库会将 `remote.origin.fetch` 设置为只跟踪一个远程分支，这意味着本地无法拉取远程仓库其他分支的信息，如有需要，则进行如下修改：
...
+refs/heads/master:refs/remotes/origin/master # fetch 时只获取远程的 master 分支，并映射到本地的 origin/master 引用
$ git config remote.origin.fetch "+refs/heads/*:refs/remotes/origin/*"
$ git config --get remote.origin.fetch
+refs/heads/*:refs/remotes/origin/* # fetch 时获取远程所有分支
$ git fetch
$ git branch -r
  origin/HEAD -> origin/master
  origin/feature/xu
