...
![](https://figure-bed.chua-n.com/notebook/Python/58.png)
...
1. 对于想设置默认参数的函数，注意如果默认参数是一个可修改的容器比如一个列表、集合或者字典，最好使用`None`作为默认值：
    ...
    | 示例                                                 | 说明                                                         |
    | ---------------------------------------------------- | ------------------------------------------------------------ |
    | ![](https://figure-bed.chua-n.com/notebook/Python/215.png) | 传递一个None值和不传值是有区别的。                           |
    | ![](https://figure-bed.chua-n.com/notebook/Python/216.png) | 默认参数的值应该是不可变的对象，比如None, True, False, 数字或字符串。<br/> 如左的作法是强烈不建议的，如果你这么做，当默认值在其他地方被修改后你将遇到各种麻烦，因为这些修改会影响到下次调用该函数时的默认值。 |
    | ![](https://figure-bed.chua-n.com/notebook/Python/217.png) | 在测试None值时使用is操作符是很重要的，这也是关键             |
...
![](https://figure-bed.chua-n.com/notebook/Python/59.png)
...
1. 关键字参数：在函数调用中前面带有标识符（例如 name=）或者作为包含在前面带有`**` 的字典里的值传入。举例来说，3 和 5 在以下对 `complex()` 的调用中均属于关键字参数:
    ...
    ![](https://figure-bed.chua-n.com/notebook/Python/60.png)

2. 位置参数：不属于关键字参数的参数。位置参数可出现于参数列表的开头以及/或者作为前面带有`*` 的 iterable 里的元素被传入。举例来说，3 和 5 在以下调用中均属于位置参数:

    ![](https://figure-bed.chua-n.com/notebook/Python/61.png)
...
1. lambda表达式允许你定义简单函数，它的使用是有限制的，只能指定单个表达式，其值就是最后的返回值。
    ...
    ![](https://figure-bed.chua-n.com/notebook/Python/218.png)
...
2. 一般地，lambda表达式中的其他变量是自由变量，在运行时绑定，而不是定义时就绑定，这不同于函数的默认值参数定义；如果想让lambda函数在定义时就捕获到值，可以将该参数定义成默认参数。
    ...
    | 定义                                                 | 使用                                                 |
    | ---------------------------------------------------- | ---------------------------------------------------- |
    | ![](https://figure-bed.chua-n.com/notebook/Python/219.png) | ![](https://figure-bed.chua-n.com/notebook/Python/220.png) |
    | ![](https://figure-bed.chua-n.com/notebook/Python/221.png) | ![](https://figure-bed.chua-n.com/notebook/Python/222.png) |
    | ![](https://figure-bed.chua-n.com/notebook/Python/223.png) | ![](https://figure-bed.chua-n.com/notebook/Python/224.png) |
...
在下面的例子中，`opener()`函数记住了`template`参数的值，并在接下来的调用中使用它：
  ...
  ![](https://figure-bed.chua-n.com/notebook/Python/225.png)

  ![](https://figure-bed.chua-n.com/notebook/Python/226.png)
...
![](https://figure-bed.chua-n.com/notebook/Python/227.png)

![](https://figure-bed.chua-n.com/notebook/Python/228.png)
...
![](https://figure-bed.chua-n.com/notebook/Python/62.png)
...
![](https://figure-bed.chua-n.com/notebook/Python/63.png)

又如，把序列[1, 3, 5, 7, 9]变成整数13579：

![](https://figure-bed.chua-n.com/notebook/Python/64.png)
...
1. 假设要按照绝对值大小排序：
    ...
    ![](https://figure-bed.chua-n.com/notebook/Python/65.png)

2. 忽略大小写的排序（默认情况下，对字符串排序是按照ASCII的大小比较的，由于`'Z' < 'a'`，导致的结果是大写字母Z会排在小写字母a的前面）：

    ![](https://figure-bed.chua-n.com/notebook/Python/66.png)

    ![](https://figure-bed.chua-n.com/notebook/Python/67.png)
...
- 返回的函数没有立刻执行，而是直到调用了f()才执行。
    ...
    ![](https://figure-bed.chua-n.com/notebook/Python/229.png)

    ![](https://figure-bed.chua-n.com/notebook/Python/230.png)

    ![](https://figure-bed.chua-n.com/notebook/Python/231.png)

- 闭包每次调用时都会返回一个新的函数，即使传入的是相同的参数。

    ![](https://figure-bed.chua-n.com/notebook/Python/232.png)
