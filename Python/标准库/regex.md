...
![](../../resources/images/notebooks/Python/270.png)
...
![](../../resources/images/notebooks/Python/271.png)
...
-   `re.DOTALL`: 结合使用 `.*` 与 `re.DOTALL`，可以让句点字符匹配所有字符，包括换行字符
    ...
    ![](../../resources/images/notebooks/Python/272-1.png)

    ![](../../resources/images/notebooks/Python/272-2.png)

-   `re.VERBOSE`: 让 re.compile()忽略正则表达式字符串中的空白符和注释，从而可以管理复杂的正则表达式。

    > Regex 中字符串的注释规则与 python 代码一样。

    ![](../../resources/images/notebooks/Python/273-1.png)

    ![](../../resources/images/notebooks/Python/273-2.png)
...
![](../../resources/images/notebooks/Python/285.png)

也可以用管道来匹配多个模式中的一个，作为正则表达式的一部分，如以下意味着试图匹配 Batman、Batmobile、Batcopter、Batbat 中任意一个：

![](../../resources/images/notebooks/Python/286.png)
...
-   `.group()`: 返回被查找字符串中实际匹配的文本。
    ...
    -   传入参数 1 或 2，代表着返回第 1 或第 2 分组的匹配文本。
        ...
        ![](../../resources/images/notebooks/Python/279-1.png)

        ![](../../resources/images/notebooks/Python/279-2.png)

-   `.groups()`：一次获取所有的分组，返回值为多个值的元组。

    ![](../../resources/images/notebooks/Python/280.png)
...
![](../../resources/images/notebooks/Python/281.png)

![](../../resources/images/notebooks/Python/282.png)

字符串搜索和替换：

-   对于复杂模式，使用`re.sub()`函数，其第一个参数是被匹配的模式，第二个参数是替换模式

    ![](../../resources/images/notebooks/Python/283.png)

    > 其中反斜杠数字比如`\3`指向前面模式的捕获组号。

-   如果除了替换后的结果外，还需要知道有多少替换发生了，使用`re.subn()`函数

    ![](../../resources/images/notebooks/Python/284.png)
...
| 符号 | 含义                                                         | 示例                                                         |
| ---- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `.`  | 匹配除了换行之外的所有字符，但只匹配一个字符                 | ![](../../resources/images/notebooks/Python/287.png)         |
| `^`  | 在Regex开始处使用^表式字符必须发生在查找文本开始处           | ![](../../resources/images/notebooks/Python/288.png)         |
| `$`  | 在Regex末尾处加上$表示匹配字符串尾，或换行符的前一个字符     | ![](../../resources/images/notebooks/Python/289.png)         |
| `*`  | 意味着匹配零次或多次，即*之前的正则式可以在文本中出现任意次  | ab* 匹配 'a', 'ab', 或'a'后面跟任意多个'b'                   |
| `+`  | 意味着匹配一次或多次，即+前面的正则式必须至少出现一次        | ab+ 匹配 'a' 后面跟1个以上到任意个 'b'：<br />![](../../resources/images/notebooks/Python/290.png) |
| `?`  | 表示它前面的正则式匹配0到1次重复                             | ab? 会匹配 'a' 或 'ab'                                       |
| `.*` | 组合使用点-星符，表示除换行外的任意文本                      | ![](../../resources/images/notebooks/Python/291.png)         |
| `^$` | 同时使用^$，表示整个字符串必须匹配该Regex模式                | ![](../../resources/images/notebooks/Python/292.png)         |
| `{}` | 指定其之前的正则式匹配固定（范围）次的重复                   | 正则表达式`(ha){3}`将匹配hahaha<br />正则表达式`(ha){3,5}`将匹配hahaha,     hahahaha 和 hahahahaha <br />正则表达式`(ha){3,}`将匹配3次及以上的文本 |
| `\`  | 转义特殊字符（允许你匹配'*', '?' 或者此类其他）              |                                                              |
| `[]` | 用[]定义自己的字符分类                                       | 如`[aeiouAEIOU]`将匹配所有元音字符 <br />如字符分类`[a-zA-Z0-9]`将匹配所有小写字母、大写字母和数字 <br />![](../../resources/images/notebooks/Python/293.png) |
