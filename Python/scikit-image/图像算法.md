...
| 代码                                                         | 作用                                                         | 补充说明                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `exposure.adjust_gamma(image, gamma=1)`                      | 伽马调整。<br />1) 若 gamma > 1，新图像变暗；<br />2) 若 gamma < 1，新图像变亮） | ![](../../resources/images/notebooks/Python/587.png)         |
| `exposure.adjust_log(image)`                                 | 对数调整                                                     | ![](../../resources/images/notebooks/Python/586.png)         |
...
-   水平边缘与垂直边缘版：
    ...
    |                        水平边缘                        |                        垂直边缘                        |
    | :----------------------------------------------------: | :----------------------------------------------------: |
    | ![](../../resources/images/notebooks/Python/588-1.png) | ![](../../resources/images/notebooks/Python/588-2.png) |
...
-   `filters.roberts_neg_diag(image)`：Roberts 的十字交叉边缘检测
    ...
    ![](../../resources/images/notebooks/Python/589.png)
...
-   `filters.threshold_otsu(image, nbins=256)`：Otsu 阈值分割（阈值 87）
    ...
    ![](../../resources/images/notebooks/Python/590.png)

-   `filters.threshold_yen(image)`：阈值 198

    ![](../../resources/images/notebooks/Python/591.png)

-   `filters.threshold_li(image)`：阈值 64.5

    ![](../../resources/images/notebooks/Python/592.png)

-   `filters.threshold_isodate(image)`：阈值 87

-   `filters.threshold_adpative(image, block_size, method='gaussian')`：此函数直接返回一个阈值后的图像，而不是阈值

    ![](../../resources/images/notebooks/Python/593.png)

    ![](../../resources/images/notebooks/Python/594.png)
...
| `sfr.bottomhat(image, selem)`        | 类似黑帽操作 | 此滤波器先计算图像的形态学闭运算，然后用原图像减去运算的结果值，有点像黑帽操作。![](../../resources/images/notebooks/Python/595.png)                 |
| `sfr.tophat(image, selem)`           | 类似白帽操作 | 此滤波器先计算图像的形态学开运算，然后用原图像减去运算的结果值，有点像白帽操作。                                                                     |
| `sfr.enhance_contrast(image, selem)` |  对比度增强  | 求出局部区域的最大值和最小值，然后看当前点像素值最接近最大值还是最小值，然后替换为最大值或最小值![](../../resources/images/notebooks/Python/596.png) |
| `sfr.entropy(image, selem)`          |   求局部熵   | 将局部区域的灰度值分布进行二进制编码，返回编码的最小值。![](../../resources/images/notebooks/Python/597.png)                                         |
| `sfr.equalize(image, selem)`         |  均衡化滤波  | 利用局部直方图对图像进行均衡化滤波![](../../resources/images/notebooks/Python/598.png)                                                               |
| `sfr.gradient(image, selem)`         |   梯度滤波   | 返回图像的局部梯度值（如最大-最小值），用此梯度值代替区域内所有像素值）![](../../resources/images/notebooks/Python/599.png)                          |
| …                                    |      ……      | ……                                                                                                                                                   |
...
-   查找轮廓：`measure.find_contours(array, level)`
    ...
    ![600](../../resources/images/notebooks/Python/600.png)

    ![](../../resources/images/notebooks/Python/601.png)
...
-   多边形逼近曲线：
    ...
    -   `measure.approximate_polygon(coords, tolerance)`
        ...
        ![](../../resources/images/notebooks/Python/602.png)
...
-   连通区域标记：`measure.label(image, connectivity=None)`
    ...
    ![](../../resources/images/notebooks/Python/603.png)
...
-   白帽：将原图像减去它的开运算值，返回比结构元小的白点
    ...
    -   `sm.white_tophat(image, selem=None)`
        ...
        ![](../../resources/images/notebooks/Python/604.png)

-   黑帽：将原图像减去它的闭运算值，返回比结构元小的黑点，且将这些黑点反色

    -   `sm.black_tophat(image, selem=None)`

        ![](../../resources/images/notebooks/Python/605.png)
...
-   求凸包
    ...
    -   将整张图片的所有目标看作一个整体：`sm.convex_hull_image(image)`
        ...
        ![](../../resources/images/notebooks/Python/606.png)

    -   求图片中每一个目标的凸包：`sm.convex_hull_object(image, neighbors=8)`

        ![](../../resources/images/notebooks/Python/607.png)
    ...
    -   删除小块区域：`sm.remove_small_objects(arr, min_size=64, connectivity=1, in_place=False)`
        ...
        ![](../../resources/images/notebooks/Python/608.png)

    -   骨架提取：`sm.skeletonize(image)`

        > 输入和输出都是一幅二值图像

        ![](../../resources/images/notebooks/Python/609.png)

        ![](../../resources/images/notebooks/Python/610.png)
    ...
    -   骨架提取（中轴变换法）：`sm.medial_axis(image, mask=None, return_distance=False)`
        ...
        ![](../../resources/images/notebooks/Python/611.png)
...
| 连通域判断方式           | from skimage import measure                             | from scipy import ndimage as ndi                             |
| ------------------------ | ------------------------------------------------------- | ------------------------------------------------------------ |
