...
1. 用字典的方式
    ...
    ![](../../resources/images/notebooks/Python/327.png)
...
2. 用一维数组的方式
    ...
    ![](../../resources/images/notebooks/Python/328.png)

### DataFrame对象的索引

1. 用字典的方式（注意，只能将列名当作字典形式的键）：

    ![](../../resources/images/notebooks/Python/329-1.png)

    ![](../../resources/images/notebooks/Python/329-2.png)
    ...
    ![](../../resources/images/notebooks/Python/330.png)
    
2. 用二维数组的方式

    - 如`data.T`可进行行列转置（视图）：

        ![](../../resources/images/notebooks/Python/331.png)

    - 获取一行数据（无行列标签）：

        ![](../../resources/images/notebooks/Python/332.png)
...
3. 注意事项
    ...
    - 如果对单个标签取值会对应列，而对多个标签用切片会对应行
        ...
        ![](../../resources/images/notebooks/Python/333.png)

    - 切片也可以不用索引值，而直接用行数来实现，即隐式索引切片

        ![](../../resources/images/notebooks/Python/334.png)

    - 同ii，掩码操作也可以直接对每一行进行过滤，而不需要使用`loc`索引器

        ![](../../resources/images/notebooks/Python/335.png)
...
- `reindex()`：索引重置
    ...
    ![](../../resources/images/notebooks/Python/363.png)

    ![](../../resources/images/notebooks/Python/364.png)

- `unstack()`：方法快速将一个多级索引的`Series`转化为普通索引的`DataFrame`

    ![](../../resources/images/notebooks/Python/365.png)

- `stack()`：方法快速将一个普通索引的`DataFrame`转化为多级索引的`Series`

    ![](../../resources/images/notebooks/Python/366.png)
...
- 为`Series`或`DataFrame`创建多级索引最直接的办法就是将`index`参数设置为至少二维的索引数组
    ...
    ![](../../resources/images/notebooks/Python/367.png)

- 若将元组作为字典的键传递给Pandas，Pandas也会默认转换为`MultiIndex`

    ![](../../resources/images/notebooks/Python/368.png)

显示地创建多级索引：

- 通过一个由不同等级的若干简单数组构成的列表创建`MultiIndex`

    ![](../../resources/images/notebooks/Python/369.png)

- 通过包含多个索引值的元组构成的列表创建`MultiIndex`

    ![](../../resources/images/notebooks/Python/370.png)

- 用两个索引的笛卡尔积创建`MultiIndex`

    ![](../../resources/images/notebooks/Python/371.png)

- 直接提供`levels`（包含每个等级的索引值列表的列表）和`labels`（包含每个索引值标签列表的列表）创建`MultiIndex`

    ![](../../resources/images/notebooks/Python/372.png)

### MultiIndex的特点

- 多级索引的**等级名称**——可以在`MultiIndex`构造器中通过`names`参数设置等级名称，也可以在创建之后通过索引的`names`属性来修改名称：

    ![](../../resources/images/notebooks/Python/373.png)

- 多级列索引——每个DataFrame的行与列都是对称的，也就是说既然有多级行索引，那么同样可以有多级列索引：

    ![](../../resources/images/notebooks/Python/374.png)

- 索引排序——局部切片和许多其他相似的操作都要求`MultiIndex`的各级索引是有序的，Pandas提供了`sort_index()`和`sortlevel()`方法进行索引排序：

    ![375](../../resources/images/notebooks/Python/375.png)

    ![](../../resources/images/notebooks/Python/376.png)
...
![](../../resources/images/notebooks/Python/377.png)

- 可以通过对多个级别索引值获取单个元素

    ![](../../resources/images/notebooks/Python/382.png)

- 可局部取值，即只索引某一个层级，结果为一个新的`Series`

    ![](../../resources/images/notebooks/Python/383.png)

- 局部切片（要求`MultiIndex`是按顺序排列的)

    ![](../../resources/images/notebooks/Python/384.png)

- 若索引已排序，可用较低层级地索引取值，第一层级可以用空切片

    ![](../../resources/images/notebooks/Python/385.png)
...
![](../../resources/images/notebooks/Python/386.png)

- 还可用`loc, iloc, ix`索引器

    ![](../../resources/images/notebooks/Python/387.png)

- 在`loc`和`iloc`中可以传递多个层级的索引元组

    ![](../../resources/images/notebooks/Python/388.png)

    > 不过这种索引元组的用法不是很方便，且在元组中使用切片会导致语法错误。

- 使用`IndexSlice`对象

    ![](../../resources/images/notebooks/Python/389.png)
...
1. `stack`与`unstack`
    ...
    | ![](../../resources/images/notebooks/Python/390.png) | ![](../../resources/images/notebooks/Python/391.png) | ![](../../resources/images/notebooks/Python/392.png) |
    | ---------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------- |

2. 用`reset_index`方法实现行列标签转换。也可以用`DataFrame`的`set_index`方法实现，返回结果就是一个带多级索引的`DataFrame`：

    | ![](../../resources/images/notebooks/Python/393.png) | ![](../../resources/images/notebooks/Python/394.png) |
    | ---------------------------------------------------- | ---------------------------------------------------- |
...
![](../../resources/images/notebooks/Python/343.png)

当在两个`Series`或`DataFrame`对象上进行二元计算时，Pandas会在计算过程中对齐两个对象的索引：

- `Series`索引对齐

    ![](../../resources/images/notebooks/Python/336.png)

    > 结果数组的索引是两个输入数组索引的并集。

- `DataFrame`索引对齐

    | ![](../../resources/images/notebooks/Python/337.png) | ![](../../resources/images/notebooks/Python/338.png) | ![](../../resources/images/notebooks/Python/339.png) |
    | ---------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------- |
...
| ![](../../resources/images/notebooks/Python/340.png) | ![](../../resources/images/notebooks/Python/341.png) | ![](../../resources/images/notebooks/Python/342.png) |
| ---------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------- |
...
![](../../resources/images/notebooks/Python/395.png)

Pandas 的`pd.concat()`函数有如下特点：

- Pandas 在合并时会保留索引，即使索引是重复的：

    ![](../../resources/images/notebooks/Python/396.png)
...
- 对于上条，虽然 DataFrame 允许这么做，但结果并不是我们想要的，`pd.concat()`提供了一些解决这个问题的方法：
    ...
    - 忽略索引：设置`ignore_index`参数为`True`，合并时会创建一个新的整数索引。
        ...
        ![](../../resources/images/notebooks/Python/397.png)

    - 增加多级索引：通过`keys`参数为数据源设置多级索引标签。

        ![](../../resources/images/notebooks/Python/398.png)

- 默认的合并方式是对所有输入列进行并集合并(`join='outer'`)，当然也可以用`join='inner'`实现对输入列的交集合并：

    | ![](../../resources/images/notebooks/Python/399.png)<br />![](../../resources/images/notebooks/Python/400.png) | ![](../../resources/images/notebooks/Python/401.png) | ![](../../resources/images/notebooks/Python/402.png) |
    | ------------------------------------------------------------ | ---------------------------------------------------- | ---------------------------------------------------- |

- 另一种合并方式是直接确定结果使用的列名，设置`join_axes`参数，里面是索引对象构成的列表（列表的列表）：

    ![](../../resources/images/notebooks/Python/403.png)

### `append()`方法

使用`append()`方法：

![](../../resources/images/notebooks/Python/404.png)
...
- 一对一连接
    ...
    | ![](../../resources/images/notebooks/Python/405.png)<br />![](../../resources/images/notebooks/Python/406.png) | ![](../../resources/images/notebooks/Python/407.png) |
    | ------------------------------------------------------------ | ---------------------------------------------------- |
...
- 多对一连接：多对一连接是指，在需要连接的两个列中，有一列的值有重复。通过多对一连接获得的结果`DataFrame`将会保留重复值。
    ...
    ![](../../resources/images/notebooks/Python/408.png)

- 多对多连接：如果左右两个输入的共同列都包含重复值，那么合并的结果就是一种多对多连接。

    ![](../../resources/images/notebooks/Python/409.png)
...
1. 用参数`on`
    ...
    | ![](../../resources/images/notebooks/Python/410.png)<br />![](../../resources/images/notebooks/Python/411.png) | ![](../../resources/images/notebooks/Python/412.png) |
    | ------------------------------------------------------------ | ---------------------------------------------------- |

2. 用`left_on`与`right_on`参数

    | ![](../../resources/images/notebooks/Python/413.png)<br />![](../../resources/images/notebooks/Python/414.png) | ![](../../resources/images/notebooks/Python/415-1.png)<br />![](../../resources/images/notebooks/Python/415-2.png) |
    | ------------------------------------------------------------ | ------------------------------------------------------------ |

    > 获取的结果中会有一个多余的列，可以通过`DataFrame`的`drop()`方法去掉：
    >
    > ![](../../resources/images/notebooks/Python/416.png)

3. 用`left_index`与`right_index`**参数**

    > 有时需合并索引，可以通过这两个参数将索引设置为键来实现合并：
    >
    > | ![](../../resources/images/notebooks/Python/417.png)<br />![](../../resources/images/notebooks/Python/418.png) | ![](../../resources/images/notebooks/Python/419.png) |
    > | ------------------------------------------------------------ | ---------------------------------------------------- |

4. 同时使用`left_index`与`right_on`（或`left_on`与`right_index`)，如此可将索引与列混合使用：

    | ![](../../resources/images/notebooks/Python/420.png)<br />![](../../resources/images/notebooks/Python/421.png) | ![](../../resources/images/notebooks/Python/422.png) |
    | ------------------------------------------------------------ | ---------------------------------------------------- |
...
| ![](../../resources/images/notebooks/Python/423.png)<br/>![](../../resources/images/notebooks/Python/424.png) | ![](../../resources/images/notebooks/Python/425.png)<br />![](../../resources/images/notebooks/Python/426.png) |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
...
![](../../resources/images/notebooks/Python/427.png)

Pandas内置的一些累计方法如下：

![](../../resources/images/notebooks/Python/428.png)
...
- 以apply为求和函数为例，GroupBy的过程如下：
    ...
    ![](../../resources/images/notebooks/Python/429.png)
...
![](../../resources/images/notebooks/Python/430.png)

- 累计：

    ![](../../resources/images/notebooks/Python/431.png)

- 过滤：过滤操作可以按照分组的属性丢弃若干数据。

    ![](../../resources/images/notebooks/Python/432.png)

- 转换：累计操作返回的是对组内全量数据缩减过的结果，而转换操作会返回一个新的全量数据。数据经过转换之后，其形状与原来的输入数据是一样的。常见的例子就是将每一组的样本数据减去各组的均值，实现标准化。

    ![](../../resources/images/notebooks/Python/433.png)

- 应用：`apply()`方法让你可以在每个组上应用任意方法。这个函数输入一个DataFrame，返回一个Pands对象(DataFrame或Series)或一个标量(scalar，单个数值)。

    ![](../../resources/images/notebooks/Python/434.png)

#### 分组案例

![](../../resources/images/notebooks/Python/435.png)

![](../../resources/images/notebooks/Python/436.png)

![](../../resources/images/notebooks/Python/437.png)
...
- DataFrame的pivot_talbe方法完整签名如下：
    ...
    ![](../../resources/images/notebooks/Python/438.png)
...
| ![](../../resources/images/notebooks/Python/440.png) | ![](../../resources/images/notebooks/Python/441-1.png)<br />![](../../resources/images/notebooks/Python/441-2.png)<br />Pandas中缺失值会被跳过。 |
| ---------------------------------------------------- | ------------------------------------------------------------ |
...
1. 与Python字符串方法相似的方法
    ...
    ![](../../resources/images/notebooks/Python/442.png)

2. 使用正则表达式的方法

    ![](../../resources/images/notebooks/Python/443.png)

3. 其他字符串方法

    ![](../../resources/images/notebooks/Python/444.png)
